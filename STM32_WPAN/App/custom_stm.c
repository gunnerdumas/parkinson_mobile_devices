...
typedef struct{
  ...
  uint16_t  CustomGet_StateHdle;                  /**< Get_State handle */
  uint16_t  CustomIo_ControlHdle;                  /**< IO_Control handle */
...
#define COPY_GET_STATE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x02,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_IO_CONTROL_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x03,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomData_ImuHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_IMU_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_DATA_IMU_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomGet_StateHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_GET_STATE_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_GET_STATE_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomGet_StateHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/

          else if (attribute_modified->Attr_Handle == (CustomContext.CustomIo_ControlHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for parkinson_data +
   *                                2 for Data_IMU +
   *                                2 for Get_State +
   *                                2 for IO_Control +
   *                                1 for Data_IMU configuration descriptor +
   *                                1 for Get_State configuration descriptor +
   *                              = 9
   *
  ...
  COPY_PARKINSON_DATA_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomParkinson_DataHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: parkinson_data, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: parkinson_data \n\r");
  }

  /**
   *  Data_IMU
   */
  COPY_DATA_IMU_UUID(uuid.Char_UUID_128);
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : DATA_IMU, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : DATA_IMU \n\r");
  }
   ...
   *  Get_State
   */
  COPY_GET_STATE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomParkinson_DataHdle,
                          UUID_TYPE_128, &uuid,
                          SizeGet_State,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomGet_StateHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : GET_STATE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : GET_STATE \n\r");
  }
   ...
   *  IO_Control
   */
  COPY_IO_CONTROL_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomParkinson_DataHdle,
                          UUID_TYPE_128, &uuid,
                          SizeIo_Control,
                          CHAR_PROP_WRITE,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomIo_ControlHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : IO_CONTROL, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : IO_CONTROL \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DATA_IMU:
      ret = aci_gatt_update_char_value(CustomContext.CustomParkinson_DataHdle,
                                       CustomContext.CustomData_ImuHdle,
                                       0, /* charValOffset */
                                       SizeData_Imu, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value DATA_IMU command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value DATA_IMU command\n\r");
      }
    ...
    case CUSTOM_STM_GET_STATE:
      ret = aci_gatt_update_char_value(CustomContext.CustomParkinson_DataHdle,
                                       CustomContext.CustomGet_StateHdle,
                                       0, /* charValOffset */
                                       SizeGet_State, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value GET_STATE command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value GET_STATE command\n\r");
      }
    ...
    case CUSTOM_STM_IO_CONTROL:
      ret = aci_gatt_update_char_value(CustomContext.CustomParkinson_DataHdle,
                                       CustomContext.CustomIo_ControlHdle,
                                       0, /* charValOffset */
                                       SizeIo_Control, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value IO_CONTROL command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value IO_CONTROL command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_DATA_IMU:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomParkinson_DataHdle, CustomContext.CustomData_ImuHdle, SizeData_Imu, pPayload);
    ...
    case CUSTOM_STM_GET_STATE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomParkinson_DataHdle, CustomContext.CustomGet_StateHdle, SizeGet_State, pPayload);
    ...
    case CUSTOM_STM_IO_CONTROL:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomParkinson_DataHdle, CustomContext.CustomIo_ControlHdle, SizeIo_Control, pPayload);
